...
PanelWindow {
    ...
    FileView {
        id: crtWatcher
        path:         Quickshell.env("HOME") + "/.cache/quickshell/crt_mode"
        watchChanges: true
        preload:      true
        onFileChanged: reload()
        onLoaded:      root._readyCrt = true
        onTextChanged: root._handleCrt(text())
    }
    ...
    function _handleCrt(raw) {
        if (!root._readyCrt) return
        var v = raw.trim().toLowerCase()
        if (v !== "on" && v !== "off") return
        if (v === "on") {
            root.iconPath    = Qt.resolvedUrl("crton.svg")
            root.labelText   = "CRT Mode"
            root.sublabel    = "On"
            root.accentColor = root.theme.accentRed
        } else {
            root.iconPath    = Qt.resolvedUrl("crtoff.svg")
            root.labelText   = "CRT Mode"
            root.sublabel    = "Off"
            root.accentColor = root.theme.textSecondary
        }
        ...
        Item {
            ...
            RowLayout {
                ...
                Item {
                    ...
                    // Soft glowing circle behind icon
                    Rectangle {
                        anchors.centerIn: parent
                        width:  54
                        height: 54
                    ...
                    Image {
                        ...
                        layer.enabled: true
                        layer.effect: null  // SVGs with own colors 
