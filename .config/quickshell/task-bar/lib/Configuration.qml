...
Scope {
    ...
    // Resolved relative to lib/usersettings.json
    // Works wherever the config directory is.
    ...
    //  External tools
    readonly property string nowPlayingBin:    Quickshell.env("HOME") + "/.config/quickshell/task-bar/bin/now_playing"
    ...
    FileView {
        ...
        preload: true
        onLoaded: { root.load(); root.writePowerMenuColors() }
    }
}
